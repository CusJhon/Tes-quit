...
async function generateQuoteCard(text, author, bgUrl = null, fontUrl = null) {
    ...
    const [fontResponse, bgResponse] = await Promise.all([fetch(fontUrlToUse), fetch(backgroundUrl)]);
    const fontBuffer = await fontResponse.arrayBuffer();
    const bgBuffer = await bgResponse.arrayBuffer();
    
    const bgBlob = new Blob([bgBuffer], { type: 'image/jpeg' });
    const bgUrlBlob = URL.createObjectURL(bgBlob);
    const bgImage = await new Promise((resolve) => { const img = new Image(); img.onload = () => resolve(img); img.src = bgUrlBlob; });
    ...
    ctx.fillText(text.substring(0, 50), canvas.width / 2, canvas.height / 2);
    ctx.font = '26px CustomFont';
    ctx.fillText(author || 'Seseorang', canvas.width / 2, canvas.height - 50);
